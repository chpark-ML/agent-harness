...
- ⏳ More language profiles — one manifest each, added when actually used. **The list was wrong until 2026-08-18**: it named `go`, `rust` and `java`, which nobody had asked for, and omitted the languages behind a stated goal. Game and app work needs `csharp-lsp` (Unity), `lua-lsp` (Godot, Roblox), `clangd-lsp` (Unreal), `swift-lsp` (iOS) and `kotlin-lsp` (Android) — **all five are already on the official marketplace**, so each is the `harness-python` shape with nothing to build. Answers §2's *game and app code gets no symbol layer* row. Add the ones actually in use, not the set.
