...
4. **A contribution is a bundle, and missing one artifact makes it unfinished.** A new hook owes six ([`CLAUDE.md`](CLAUDE.md) §2): the script, a verifier with 8+ cases, `docs/hooks/<name>.md`, registration in `hooks/hooks.json` anchored on `${CLAUDE_PLUGIN_ROOT}`, an update to `docs/agent-layer.md`, and a `version` bump. A skill owes a trigger eval (§2b). A `bin/` executable owes a shim in `install.sh` rather than a `hooks.json` entry (§2c). An output style owes the `outputStyle` scalar that selects it — shipping the file selects nothing — and an explicit `keep-coding-instructions`, whose `false` default silently drops the built-in coding instructions (§2e). The `<name>` must be identical everywhere or the audit cannot run mechanically.
